...
impl VecBuffer {
    fn read_exact(&mut self, buf: &mut [u8]) -> Result<(), Error> {
        if let Some(target_bytes) = self.buffer.get(self.offset..self.offset + buf.len()) {
...
fn read_u32(read: &mut VecBuffer) -> Result<u32, Error> {
    let mut buffer = [0; 4];
...
impl Image {
    ...
    fn read(read: &mut VecBuffer, expected_kind: u32, expected_size: u32) -> Result<Self, Error> {
...
impl TocEntry {
    ...
    fn read(read: &mut VecBuffer) -> Result<Self, Error> {
        Ok(TocEntry {
            kind: read_u32(read)?,
            size: read_u32(read)?,
...
pub(crate) fn parse_cursor(input: &mut VecBuffer, desired_size: u32) -> Result<Vec<Image>, Error> {
    ...
    for entry in toc {
        ...
        input.seek(entry.position as usize)?;
...
mod test {
    ...
    fn read_3x5_image() {
        ...
        let image = Image::read(&mut vb, IMAGE_TYPE, 4).unwrap();
    ...
    fn parse_cursor_one_image() {
        ...
        let mut vb = VecBuffer::new(data.to_vec());
        let actual = parse_cursor(&mut vb, 10).unwrap();
